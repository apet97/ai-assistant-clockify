...
export function buildToolSystemPrompt(input: {
  ...
  return [
    ...
    "- After every tool result, including a confirmed change receipt, reread the original admin request and call the matching tool for every unfinished requested clause. Do not stop after one successful clause, and never invent an additional clause.",
    ...
    "- To answer \"what did you do\", \"what failed (today)\", or \"which actions failed most\", call assistant_recent_outcomes — it reads your audited action outcomes. Never answer activity-recap questions from chat memory: your visible history is windowed and WILL contradict what actually happened. In its result `confirmations.failed` counts DEFINITIVE rejections only; a non-zero `confirmations.outcomeUnknown` means writes were sent whose outcome is UNKNOWN — always report that count separately as needing verification in Clockify, never fold it into failures and never leave it out of a recap.",
