...
export async function discoverItemTypes(
  ...
  const scanned = all.rows.slice(0, MAX_INVOICES_SCANNED_FOR_TYPES);
  const names = new Set<string>();
  let truncated = all.truncated || all.rows.length > scanned.length;
  ...
  for (let i = 0; i < scanned.length; i += ITEM_TYPE_SCAN_CONCURRENCY) {
    ...
    for (const items of itemLists) {
      ...
      for (const it of items.rows) if (it.itemType) names.add(it.itemType);
    }
  }
...
export type ItemTypeResolution =
  ...
  | { ok: false; options: string[]; incomplete?: boolean };
 ...
 */
export function resolveItemType(discovered: ListResult<string>, requested?: string): ItemTypeResolution {
  const match = requested
    ? discovered.rows.find((itemType) => itemType.toLowerCase() === requested.toLowerCase())
    : undefined;
  if (match) return { ok: true, itemType: match };
  if (discovered.truncated) return { ok: false, options: discovered.rows, incomplete: true };
  if (discovered.rows.length === 0) return { ok: true, itemType: requested ?? "NEW DEFAULT" };
  if (!requested) return { ok: true, itemType: discovered.rows[0] };
  return { ok: false, options: discovered.rows };
}

export function itemTypeClarify(options: string[], incomplete = false): { clarify: string; options: { id: string; label: string }[] } {
  return {
    clarify: incomplete
      ? `Clockify returned an incomplete invoice item-type list, so I can't prove which type to use. Provide the exact configured item type or narrow the invoice set.`
      : `This workspace's invoice item types are: ${options.join(", ")}. Which should I use? (I can't create a new type via the API — add a line item once in the Clockify invoice editor and it auto-creates a type I can then reuse.)`,
