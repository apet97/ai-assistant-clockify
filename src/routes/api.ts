...
export function apiRouter(
  ...
  router.post("/confirmation-batches/:id/confirm", sessionAsyncHandler(async (req, res) => {
    ...
    if (stream) {
      const { write, signal } = openNdjsonStream(res);
      try {
        const committed = await withHostCallBudget(() => confirmationService.confirmBatch({
          claims,
          batchId: batch.id,
    ...
    let committed: Awaited<ReturnType<typeof confirmationService.confirmBatch>>;
    try {
      committed = await withHostCallBudget(() => confirmationService.confirmBatch({
        claims,
        batchId: batch.id,
        items: parsed.data.items,
        signal: requestAbort.signal,
