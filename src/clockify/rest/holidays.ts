...
export function makeHolidayRest(core: RestCore, workspaceId: string): HolidayPort {
  ...
  const prepareUpdate = async (id: string, patch: UpdateHolidayInput): Promise<PreparedHolidayUpdateInput> => {
    ...
    return { name: patch.name ?? existing.name, startDate: patch.startDate ?? existing.datePeriod.startDate, endDate: patch.endDate ?? patch.startDate ?? existing.datePeriod.endDate ?? existing.datePeriod.startDate, ...(patch.occursAnnually ?? existing.occursAnnually) != null ? { occursAnnually: patch.occursAnnually ?? existing.occursAnnually } : {}, ...(userIds.length ? { userIds } : {}), ...(userGroupIds.length ? { userGroupIds } : {}), ...(existing.everyoneIncludingNew != null ? { everyoneIncludingNew: existing.everyoneIncludingNew } : {}), source: mapHoliday(existing as Record<string, unknown>) };
  };
