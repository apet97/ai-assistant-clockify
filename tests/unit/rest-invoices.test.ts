...
describe("invoice rest", () => {
  ...
  it("listInvoicePayments unwraps the {payments:[…]} envelope", async () => {
    const f = vi.fn(async () =>
  ...
  it("createInvoicePayment POSTs amount + paymentDate (NOT date)", async () => {
    const f = vi.fn(async () => jsonResponse({ id: "pay1" }));
    const out = await rest(f as unknown as typeof fetch).createInvoicePayment("inv1", {
      amountMinor: 5000,
      paymentDate: "2026-06-06",
      note: "deposit",
    });
    expect(out).toMatchObject({ id: "pay1" });
    const [url, init] = (f as any).mock.calls[0];
    expect(url).toBe("https://api.clockify.me/api/v1/workspaces/ws-1/invoices/inv1/payments");
    expect(init.method).toBe("POST");
