...
CREATE TABLE pending_confirmations (
      ...
      agent_state_json TEXT, capability_id TEXT, capability_hash TEXT, origin TEXT CHECK (origin IS NULL OR origin IN ('assistant','direct_ui','system','live_test')), registry_id TEXT CHECK (registry_id IS NULL OR registry_id IN ('v1-internal','v2-api','v2-local')), authority_model TEXT CHECK (authority_model IS NULL OR authority_model IN ('legacy_v1','intent_capability_v1','preview_confirmation_v2','trusted_direct_v2','undo_v2')), executor_kind TEXT CHECK (executor_kind IS NULL OR executor_kind IN ('legacy_v1','prepared_safe_write','risky_commit','direct_safe_write','undo_commit')), run_id TEXT, batch_id TEXT, installation_generation INTEGER CHECK (installation_generation >= 1),
      FOREIGN KEY (session_id) REFERENCES chat_sessions(id)
    );
...
CREATE UNIQUE INDEX idx_assistant_run_request_links_one_initial
    ON assistant_run_request_links(session_id, run_id)
    WHERE kind = 'initial';

CREATE TABLE turn_message_links (
    session_id TEXT NOT NULL,
    request_id TEXT NOT NULL,
