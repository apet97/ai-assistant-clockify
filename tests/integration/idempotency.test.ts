...
describe("idempotent commits (Phase 5)", () => {
  ...
  it("does not create a duplicate invoice when the same intent is confirmed twice", async () => {
    ...
    expect(fake.counts.createInvoiceBase).toBe(1); // the second confirm did NOT create a second invoice
  ...
  it("dedupes one stored operation even if volatile invoice fields are reconstructed differently", async () => {
    ...
    const payload1 = op1.payload as { base: Record<string, unknown>; items: unknown };
    const op2: ConfirmableOperation = {
      ...op1,
      payload: {
        ...payload1,
        base: { ...payload1.base, number: "INV-TOTALLY-DIFFERENT", issuedDate: "2099-01-01T00:00:00.000Z" },
      },
    };
    ...
    expect(fake.counts.createInvoiceBase).toBe(1); // same operation identity, so still one dispatch
  });
  ...
  it("does NOT replay across admins — the ledger is shared but the scope key is per-admin", async () => {
    ...
    expect(fake.counts.createInvoiceBase).toBe(2); // each admin got their own invoice
  ...
  it("does NOT replay across workspaces — the scope key is per-workspace", async () => {
    ...
    expect(r1.ok && r2.ok).toBe(true);
    expect(fake.counts.createInvoiceBase).toBe(2); // each workspace got its own invoice
  ...
  it("without a ledger in context, behaves exactly as before (no dedup)", async () => {
    ...
    await commitConfirmedOperation(ctx, op);
    expect(fake.counts.createInvoiceBase).toBe(2); // unchanged behavior without a ledger
  });
  ...
  it("a FAILED commit is not recorded — the retry actually executes and creates the entity exactly once", async () => {
    ...
    const fake = createFakeWorkspace({ clients: [{ id: "c-qwen", name: "qwen" }] });
    let createCalls = 0;
    const realCreateInvoiceBase = fake.client.createInvoiceBase.bind(fake.client);
    // Wrap the client so the atomic base POST fails definitively on its FIRST call only, then works.
    const flakyClient = new Proxy(fake.client, {
      get(target, prop, receiver) {
        if (prop === "createInvoiceBase") {
          return (...args: Parameters<typeof realCreateInvoiceBase>) => {
            createCalls += 1;
            if (createCalls === 1) {
              throw new DefinitiveWriteFailure("POST", "/workspaces/ws-1/invoices", "host rejected invoice", 400);
            }
            return realCreateInvoiceBase(...args);
          };
        }
        return Reflect.get(target, prop, receiver);
      },
    ...
    expect(fake.counts.createInvoiceBase).toBe(1); // exactly one invoice exists
