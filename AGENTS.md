...
Full build history: [`docs/V2_BUILD_LOG.md`](./docs/V2_BUILD_LOG.md).
Authoritative engineering detail: [`CLAUDE.md`](./CLAUDE.md) — this file is the
short map, and never duplicates it.

- **Deployed:** `ec09863`, `ASSISTANT_ENGINE=v2`, schema 12 at
  `/data/ai-assistant.sqlite`. v1 stays in-tree for rollback.
- **Catalog:** 171 actions — `api` 127 · `composite` 24 · `generic` 16 · `local` 4.
- **Reads work end to end.** Discovery → execute → results fed back → grounded
  answer, proven in production.
- **Writes do not reach the admin.** `v2OutcomeToTurn` returns `results: []` on
  every arm, so a prepared write renders "Review the preview and click Confirm."
  with no card and no button. Top open defect.
- **Also open:** the presentation layer is unwired (`run-event-hydration.ts`
  hardcodes `facts: []`/`references: []` and uses raw action names as titles).
- **Four defects fixed 2026-07-27/28** — tool-result feedback, the missing `runId`
  on the write port, the discarded model reply, and a fatal third discovery
  search. Three hid behind test fakes more permissive than the real store; check
  that first when a v2 test passes.
