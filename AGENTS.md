...
- T06-ENTRY-UPDATE CLOSED: `clockify_entries_update` (`updateTimeEntry`), `clockify_entries_delete`, bounded `clockify_entries_mark_invoiced` (`updateInvoicedStatus`, max 21); `clockify_fix_entry` stays off MODEL_API. `MODEL_API` = 103; `ACTION_CATALOG` = 157. Live: `live_not_run_missing_credentials`. Next: `T06-REPORTS`.
