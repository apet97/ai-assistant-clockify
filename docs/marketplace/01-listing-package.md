# Marketplace listing package - version 1.0.0
...
> the assistant maps the request to a fixed catalog of typed Clockify actions.
>
> The model does not receive Clockify credentials and cannot execute an action itself.
> A deterministic backend verifies the administrator role, the administrator's
> saved permission policy, the action schema, the declared intent, and the operation's
...
| Demo video | Reproducible release-build recording | `assets/video/ai-assistant-1.0.0-demo.mp4` | Script below, readable UI, no secrets or customer data |
...
3. Run one safe write in the sacrificial workspace and show the immediate receipt.
...
- Confirm version 1.0.0 in package/portal metadata and the deployed `/version` release
...
- Confirm the final pricing value is **Free add-on; Clockify Pro required** and paste the
  prepared [`What's New`](./04-whats-new-1.0.0.md) version entry.
